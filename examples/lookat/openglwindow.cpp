...
void OpenGLWindow::handleEvent(SDL_Event& ev) {
  if (ev.type == SDL_KEYDOWN) {
    ...
    if (ev.key.keysym.sym == SDLK_r) m_heightSpeed = -1.0f;
    if (ev.key.keysym.sym == SDLK_f) m_heightSpeed = 1.0f;
  }
  if (ev.type == SDL_KEYUP) {
    ...
    if (ev.key.keysym.sym == SDLK_r && m_heightSpeed < 0) m_heightSpeed = 0.0f;
    if (ev.key.keysym.sym == SDLK_f && m_heightSpeed > 0) m_heightSpeed = 0.0f;
  }
}
...
void OpenGLWindow::update() {
  ...
  m_camera.height(m_heightSpeed * deltaTime);
}